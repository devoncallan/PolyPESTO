...
# from .params import ParameterSet, ParamID
...
#         return Conditions(
#             exp_id=d["exp_id"],
#             values=ParameterSet.from_dict(d["values"]),
#         )
...
#     def __getitem__(self, key: ParamID) -> float:
#         return self.values[key]


# @dataclass
# class SimConds(Conds):
...
#             exp_id=d["exp_id"],
#             values=ParameterSet.from_dict(d["values"]),
#             true_params=ParameterSet.from_dict(d["true_params"]),
...
# # def create_conditions(conds: Dict[str, ArrayLike]) -> List[ParameterSet]:

# #     cond_labels = list(conds.keys())
# #     pass


# # def create_conds(conds: Dict[ParamID, ArrayLike]) -> None:

# #     param_names = list(conds.keys())
# #     conditions = {param_id: np.array(values) for param_id, values in conds.items()}
# #     len_conds = {k: len(np.array(v)) for k, v in conditions.items()}
# #     n_conds = len_conds[param_names[0]]
...
# # def conditions_to_df(conds: Sequence[ParameterSet]) -> pd.DataFrame:

# #     return pet.define_conditions(
# #         [cond.to_dict() for cond in conds], exp_ids=[cond.id for cond in conds]
# #     )
