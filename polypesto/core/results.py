...
class ParameterResult:
    ...
    def __init__(self, result, true_params=None, parameter_indices=None):
        self.result = result
    ...
    def _get_parameter_names(self):
        ...
        if self.parameter_indices == 'free_only':
            ...
            if hasattr(self.result.problem, 'lb') and hasattr(self.result.problem, 'ub'):
                indices = [i for i, (lb, ub) in enumerate(zip(self.result.problem.lb, self.result.problem.ub)) 
                          if lb != ub]
                return [self.result.problem.x_names[i] for i in indices 
                       if i < len(self.result.problem.x_names)]
            return self.result.problem.x_names
            
        # Normal case - numeric indices    
        return [self.result.problem.x_names[i] for i in self.parameter_indices 
                if i < len(self.result.problem.x_names)]
    ...
    def get_scaled_value(self, param_id, value):
        ...
        """
        if not hasattr(self.result.problem, 'x_names') or param_id not in self.result.problem.x_names:
            return value
        
        idx = self.result.problem.x_names.index(param_id)
        if hasattr(self.result.problem, "x_scales") and idx < len(self.result.problem.x_scales) and self.result.problem.x_scales[idx] == "log10":
...
class OptimizationResult(ParameterResult):
    ...
    def get_best_parameters(self, scaled=True):
        ...
        """
        if not hasattr(self.result, 'optimize_result') or self.result.optimize_result is None:
            return {}
...
class ProfileResult(ParameterResult):
    ...
    def __init__(self, result, true_params=None, parameter_indices=None, profile_list_id=0):
    ...
    def has_profile_results(self):
        ...
        return (hasattr(self.result, 'profile_result') and 
                self.result.profile_result is not None)
    ...
    def get_profile_indices(self):
        ...
        if hasattr(self.result.profile_result, 'profile_x') and len(self.result.profile_result.profile_x) > self.profile_list_id:
            return list(range(len(self.result.profile_result.profile_x[self.profile_list_id])))
        # Older PyPESTO versions
        elif hasattr(self.result.profile_result, 'x_path') and len(self.result.profile_result.x_path) > self.profile_list_id:
            return list(range(len(self.result.profile_result.x_path[self.profile_list_id])))
...
class SamplingResult(ParameterResult):
    ...
    def has_sampling_results(self):
        ...
        return (hasattr(self.result, 'sample_result') and 
                self.result.sample_result is not None)
